...
def apply_base_gate_with_noise(st, gate_id, q1, q2, n_qubits, device, noise_model: TorchNoisyGates|None):
    ...
    H = torch.tensor([[1/math.sqrt(2),1/math.sqrt(2)],
                      [1/math.sqrt(2),-1/math.sqrt(2)]],dtype=torch.complex64,device=device)
    ...
    if gate_id == gate_mapping['h']:
        U = build_full_single(H,q1,n_qubits,device); st = U @ st; acted=[q1]
    elif gate_id == gate_mapping['x']:
        U = build_full_single(Xg,q1,n_qubits,device); st = U @ st; acted=[q1]
    elif gate_id == gate_mapping['z']:
        U = build_full_single(Zg,q1,n_qubits,device); st = U @ st; acted=[q1]
    elif gate_id == gate_mapping['cx'] and (q2 < n_qubits):
        U = build_cx(q1,q2,n_qubits,device); st = U @ st; acted=[q1,q2]
    elif gate_id == gate_mapping['cz'] and (q2 < n_qubits):
        U = build_cz(q1,q2,n_qubits,device); st = U @ st; acted=[q1,q2]
    if noise_model is not None and noise_model.has_noise() and acted:
        ...
        for i,qb in enumerate(acted):
            Rx = single_Rx(x_angles[i].unsqueeze(0))[0]
            Rz = single_Rz(z_angles[i].unsqueeze(0))[0]
            Ux = build_full_single(Rx,qb,n_qubits,device)
            Uz = build_full_single(Rz,qb,n_qubits,device)
            st = Uz @ (Ux @ st)
    return st
...
def simulate_interleaved_with_params(base_gate_ids, base_q1, base_q2,
    ...
    def apply_param(st, q, t_idx, theta):
        if t_idx == param_gate_mapping['rz']:
            U1 = single_Rz(theta.view(1))[0]
        else:
            U1 = single_Rx(theta.view(1))[0]
...
if __name__ == "__main__":
    model = main_training_loop(tokenized_data_x, extracted_sched,
                               ...
                               run_final_fidelity=True, save_path="best_model_100g.pt")
