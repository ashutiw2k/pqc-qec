...
# ---- 批量与优化相关（已为大批量 400 调整）----
BASELINE_BS = 32          # 你之前稳定的小批量
BATCH_SIZE  = 32         # 目标批量（大显存 GH200）
LR_BASE     = 1e-4        # 在 batch=32 时的基线学习率
LR_SCALE    = BATCH_SIZE / BASELINE_BS
LR          = LR_BASE * LR_SCALE * 0.5  # 先保守乘 0.5，稳定后可调到 *1.0
GRAD_CLIP   = 1.0
...
USE_FUSED_BASE_NOISE = True  # 实验: 使用 CUDA fused kernel 加速 base+noise
DEBUG_VALIDATE_FUSED = False  # 默认关闭；如需重新验证设 True
...
"""
    ...
    extra_cuda_flags = ["-allow-unsupported-compiler", "-O3", "--use_fast_math"]
