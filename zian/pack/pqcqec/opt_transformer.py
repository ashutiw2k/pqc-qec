...
# -*- coding: utf-8 -*-
"""
Transformer-based angle prediction module extracted from newest_work_correct.
Provides:
- AnglePredictor model (predicts sin/cos for param angles)
- train_transformer: training loop using shared-PQC layout simulator
"""
...
from .simulator_core import (
    ...
    simulate_loss, build_base_cache_vectorized, NoiseConfig,
...
class AnglePredictor(nn.Module):
    """Autoregressive per-qubit predictor based on per-block base-gate counts.

    For each target qubit line, at block t it only sees:
    - cumulative base gate count before block t (for that qubit)
    - block index t
    - previous predicted angle (rz1, rx, rz2) from block t-1 (initialized as zeros)

    It unrolls GRU over blocks and outputs [B, max_blocks*3, 1] (padded with zeros beyond T_i).
    """
    def __init__(self, gate_blocks: int, use_quaternion_head: bool = False):
        ...
        # Per-step features: [count_t, cum_t, t_index, prev_angles_window_flat(K*3)]
        feat_dim = 3 + 3 * PREV_K
        ...
        # Two head options:
        # 1) S¹ per-angle head (6D: [x,y] per angle)
        self.head = nn.Linear(HID_DIM, 6)
        # 2) Quaternion head (4D: [w,x,y,z]) then convert to ZXZ Euler (rz1, rx, rz2)
...
def train_subcircuits(data_path: str,
    ...
    if VERBOSE:
        try:
            ...
            pass
    # dataset: per-qubit single-qubit circuits
    ...
    def _grouped_blocks_loss(b: Batch, logits: torch.Tensor) -> torch.Tensor:
        ...
        for L in uniq_L.tolist():
            ...
            lval = simulate_loss(b_sub, l_sub, init_cache, ref_cache, noise_schedules,
                                 mode='blocks', gate_blocks=gate_blocks, detach_base_noise=detach_base_noise)
            # lval is a scalar tensor; append to list to keep graph
            losses.append(lval)
        if not losses:
        ...
        with torch.no_grad():
            for bi, b in enumerate(val_loader):
                ...
                for L in uniq_L.tolist():
                    ...
                    lval = simulate_loss(b_sub, l_sub, init_cache, ref_cache, noise_schedules,
                                         mode='blocks', gate_blocks=gate_blocks, detach_base_noise=detach_base_noise)
                    group_fids.append(float(1.0 - lval.item()))
                if group_fids:
                    total += sum(group_fids) / len(group_fids); m += 1
        model.train()
                ...
                if VERBOSE and (val_loader is not None):
                    ...
                    try:
                        ...
                        if (not model.use_ordered_seq) and Bsz >= 3:
                            ...
                            pre_steps, post_steps = model.get_step_embeddings(vcounts, vT, max_steps=maxS, extra_feats=vhist3, hist_ctx=ctx)
                            def cos(a,b):
                    ...
                    try:
                        ...
                        if Bsz >= 3:
                            ...
                            pre_steps, post_steps = model.get_step_embeddings(tcounts, tT, max_steps=maxS, extra_feats=thist3, hist_ctx=ctx)
                            def cos(a,b):
                                return float(F.cosine_similarity(a.unsqueeze(0), b.unsqueeze(0)).item())
                            step_idx = 24
                            S = pre_steps.size(1)
