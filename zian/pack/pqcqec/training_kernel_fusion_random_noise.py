...
K_RANDOM=100  # 减少随机初态数量，10 qubits复杂度高

# ---- 批量与优化相关（调整为10 qubits）----
BASELINE_BS = 32          # 你之前稳定的小批量
BATCH_SIZE  = 32          # 10 qubits需要更小的batch size，内存约需32倍
...
# ===== 新增：每个epoch使用不同噪声 =====
USE_EPOCH_VARYING_NOISE = True  # 为每个epoch生成不同的噪声
# ---- 额外性能调试 & 减少 CPU 同步 ----
...
def build_base_cache_vectorized(dataset: CircuitDataset):
    ...
    if FAST_NOISE_SCHEDULE:
        ...
        if USE_NOISE:
            if USE_EPOCH_VARYING_NOISE:
                # 为每个epoch生成不同的噪声: shape [EPOCHS, B_total, L_max_global]
                print(f'[INFO] Generating epoch-varying noise for {EPOCHS} epochs, {B_total} circuits...')
                rx_q1 = torch.zeros(EPOCHS, B_total, L_max_global, device=device)
                rz_q1 = torch.zeros(EPOCHS, B_total, L_max_global, device=device)
                rx_q2 = torch.zeros(EPOCHS, B_total, L_max_global, device=device)
                rz_q2 = torch.zeros(EPOCHS, B_total, L_max_global, device=device)
                ...
                for epoch in range(EPOCHS):
                    ...
                    rx_q1[epoch] = torch.where(rx_flag1, rx_amp1, torch.zeros(1, device=device))
                    ...
                    rz_q1[epoch] = torch.where(rz_flag1, rz_amp1, torch.zeros(1, device=device))
                    ...
                    rx_q2[epoch] = torch.where(rx_flag2, rx_amp2, torch.zeros(1, device=device))
                    ...
                    rz_q2[epoch] = torch.where(rz_flag2, rz_amp2, torch.zeros(1, device=device))
                
                print(f'[INFO] Generated noise tensor shapes: rx_q1={tuple(rx_q1.shape)}, memory={rx_q1.numel()*4/1024/1024:.1f}MB per tensor')
            else:
                # 原来的实现：所有epoch使用相同噪声
                rx_flag1 = (torch.rand(B_total, L_max_global, device=device) < NOISE_DELTA_X) & gate_mask
                rx_amp1  = (torch.rand(B_total, L_max_global, device=device)*2 - 1) * NOISE_X_RAD
            ...
            # 无噪声情况
            if USE_EPOCH_VARYING_NOISE:
                rx_q1 = rz_q1 = rx_q2 = rz_q2 = torch.zeros(EPOCHS, B_total, L_max_global, device=device)
            else:
        ...
        noise_schedules = dict(tensor_mode=True, rx_q1=rx_q1, rz_q1=rz_q1, rx_q2=rx_q2, rz_q2=rz_q2,
                               ...
                               epoch_varying=USE_EPOCH_VARYING_NOISE, num_epochs=EPOCHS if USE_EPOCH_VARYING_NOISE else 1)
...
def get_epoch_noise_schedules(noise_schedules: dict, epoch: int) -> dict:
    """从多epoch噪声中提取特定epoch的噪声"""
    if not noise_schedules.get('epoch_varying', False):
        # 如果不是epoch变化的噪声，直接返回原噪声
        return noise_schedules
    
    # 提取第epoch个噪声 (epoch从1开始，但数组从0开始)
    epoch_idx = (epoch - 1) % noise_schedules['num_epochs']
    
    return dict(
        tensor_mode=True,
        rx_q1=noise_schedules['rx_q1'][epoch_idx],  # [B_total, L_max_global]
        rz_q1=noise_schedules['rz_q1'][epoch_idx],
        rx_q2=noise_schedules['rx_q2'][epoch_idx], 
        rz_q2=noise_schedules['rz_q2'][epoch_idx],
        idx2row=noise_schedules['idx2row'],
        L_max=noise_schedules['L_max'],
        epoch_varying=False,  # 单epoch噪声不再是变化的
        current_epoch=epoch
    )
...
def train():
    ...
    for ep in range(1,EPOCHS+1):
        ...
        # 获取当前epoch的噪声
        if PRECOMPUTE_BASE and USE_EPOCH_VARYING_NOISE:
            current_noise_schedules = get_epoch_noise_schedules(noise_schedules, ep)
            print(f'[INFO] Epoch {ep}: Using noise set {ep} (shape: {tuple(current_noise_schedules["rx_q1"].shape)})')
        else:
            current_noise_schedules = noise_schedules
            ...
            if PRECOMPUTE_BASE:
                if FAST_PARAM_SIM and shared_param_layout is not None:
                    if AGGREGATE_PARAM_ANGLES:
                        main = simulate_loss_shared_param_fast_agg(batch, logits, init_cache, ref_cache, current_noise_schedules, shared_param_layout)
                    else:
                        main = simulate_loss_shared_param_fast(batch, logits, init_cache, ref_cache, current_noise_schedules, shared_param_layout)
                else:
                    main = simulate_loss_cached_vectorized_samepqc(batch, logits, init_cache, ref_cache, current_noise_schedules)
                ...
                if 'DEBUG_VALIDATE_FUSED' in globals() and DEBUG_VALIDATE_FUSED and ep==1 and bi <= 3:
                    if USE_FUSED_BASE_NOISE:
                        ...
                        with torch.no_grad():
                            ref_main = simulate_loss_shared_param_fast_agg(batch, logits, init_cache, ref_cache, current_noise_schedules, shared_param_layout) if AGGREGATE_PARAM_ANGLES else simulate_loss_shared_param_fast(batch, logits, init_cache, ref_cache, current_noise_schedules, shared_param_layout)
