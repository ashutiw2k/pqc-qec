...
# Choose decorator based on CUDA availability
# njit is faster when CUDA is available, jit is more compatible otherwise
jit_decorator = njit if HAS_CUDA else jit

# Gate Enums as regular ints
...
# Move all functions outside the class and make them standalone jit functions
@jit_decorator
...
def build_numba_circuit(circuit_ops, dtype=np.float32):
    """
...
@jit_decorator
def create_ones_state(n_qubits):
